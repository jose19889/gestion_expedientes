...
public function descargar($id)
{
    ...
    try {
        ...
        $dossier = $dossierModel->find($id);

        if (!$dossier) return redirect()->back()->with('danger', 'Expediente no encontrado.');

        // --- CONFIGURACIÓN DE RUTAS ---
        $qrNombre = basename(trim($dossier->codigo_qr)) ?: "exp_{$id}.png";
        if (!str_contains(strtolower($qrNombre), '.png')) $qrNombre .= '.png';
        $qrPath = WRITEPATH . 'uploads/qrs/' . $qrNombre;

        // --- INTENTO DE GENERACIÓN CON GOOGLE (SI HAY RED) ---
        $infoTexto = "TITULO: " . ($dossier->titulo ?? 'N/A') . "\n";
        $infoTexto .= "CODIGO: " . ($dossier->codigo ?? 'N/A') . "\n";
        $infoTexto .= "ESTADO: " . strtoupper($dossier->estado ?? 'PENDIENTE');
        
        $urlTexto = urlencode($infoTexto);
        $googleChartApi = "https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl={$urlTexto}&choe=UTF-8";

        // Intentamos conectar con Google con un tiempo de espera corto (2 segundos)
        $context = stream_context_create(['http' => ['timeout' => 2]]); 
        $nuevoQr = @file_get_contents($googleChartApi, false, $context);

        if ($nuevoQr !== false) {
            // SI HAY RED: Sobrescribimos con el texto informativo
            if (!is_dir(dirname($qrPath))) mkdir(dirname($qrPath), 0777, true);
            file_put_contents($qrPath, $nuevoQr);
        } 
        // SI NO HAY RED: El código no hace nada y saltará al siguiente paso usando el archivo existente

        // --- PROCESO BASE64 PARA EL PDF ---
        $qrBase64 = '';
        if (file_exists($qrPath)) {
            $data = file_get_contents($qrPath);
            $qrBase64 = 'data:image/png;base64,' . str_replace(["\r", "\n"], '', base64_encode($data));
        }

        // --- RENDERIZADO DEL PDF ---
        $html = view('dossier/pdf_template', [
            'dossier'            => $dossier,
            'archivos'           => (new \App\Models\FileModel())->where('expediente_id', $id)->findAll(),
            'qrBase64'           => $qrBase64,
            'departamentoNombre' => $dossier->departamento_actual
        ]);
        ...
        header('Content-Disposition: attachment; filename="' . $dossier->codigo . '.pdf"');
